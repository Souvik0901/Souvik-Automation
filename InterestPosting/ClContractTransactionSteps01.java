...
public class ClContractTransactionSteps {
    ...
    @Given("Open Interst Posting Transaction with index {int}")
    public void openInterstPostingTransactionWithIndex(int index) {
        transactionPage.openInterstPostingTransactionByIndex(index);
        transactionPage.switchToDefaultContent();
    }
    ...
    @Given("Open Accounting Entries For Interest Posting Transaction with index {int}")
    public void openAccountingEntriesForInterstPostingWithIndex(int index) {
        transactionPage.waitForElementToBeVisible(transactionPage.relatedButtonInterestPosting, 20);
        transactionPage.refreshTheBrowser();
        transactionPage.clickUsingJavaScript(transactionPage.relatedButtonInterestPosting);
        transactionPage.scrollDownIntoView(transactionPage.accountEntriesHeader);        
        transactionPage.waitForElementToBeVisible(transactionPage.accountEntriesHeader, 10);
        
         By accountingEntryLinkId = By
        .xpath(String.format(transactionPage.dynamicAccountingEntryLink, index));
        transactionPage.clickUsingJavaScript(accountingEntryLinkId);
    }
